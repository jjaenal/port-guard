  ...
  describe('getSimplePrices', () => {
    it('fetches prices correctly with default currency', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('ids=ethereum%2Cmatic-network&vs_currencies=usd'),
        expect.anything()
      );
    ...
    it('handles API errors gracefully', async () => {
      ...
      await expect(getSimplePrices(['ethereum'])).rejects.toThrow('CoinGecko request failed: 429');
      expect(global.fetch).toHaveBeenCalledTimes(1);
    });
